...
NEWSCHEMA('Posts', function(schema) {
	...
	schema.define('name_fr', String, true);
	schema.define('author', 'String(30)', true);
	schema.define('description', 'String(300)');
	schema.define('description_fr', 'String(300)');
	schema.define('summary', 'String(500)');
	schema.define('summary_fr', 'String(500)');
	...
	schema.define('tags_fr', '[String(30)]');
	...
	schema.setSave(function($, model) {
		...
		model.tags = model.keywords ;
		model.tags_fr = model.keywords_fr ;
		model.tags_fr = model.keywords_fr ;
		model.content = model.description ;
		model.content_fr = model.description_fr ;
		...
		model.search = ((model.name || '') +(model.name_fr || '') +(model.keywords_fr || '') + ' ' + (model.keywords || '') + ' ' + (model.search || '')).keywords(true, true).join(' ').max(1000);
